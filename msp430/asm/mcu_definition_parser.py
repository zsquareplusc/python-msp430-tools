...
segment <name>  <memory_range>
    ...
    <memory_range> can be like "0x0200-0x0300" or a set of "key:value" pairs.

    Supported pairs are: "in:<segmen_name>" "mirror:<segment_name>"
...
symbol <name> <address>
    ...
    range specified in the definion (opposed to the values after the linker has
...
memory-map-begin
    ...
    'name' and 'segment' commands are used to define a memory map.

memory-map-end
    Terminate memory map definition.
...
class MCUDefintitions(rpn.RPN):
    ...
    def word_PROGRAMMABLE(self, stack):
        """Set flag for the following SECTION"""
    ...
    def word_MEMORY_MAP_END(self, stack):
        """Terminate current memory map definition"""
    ...
    def word_SEGMENT(self, stack):
        """Define a segment. Consume flags that have been set"""
        if self.memory_map is None:
